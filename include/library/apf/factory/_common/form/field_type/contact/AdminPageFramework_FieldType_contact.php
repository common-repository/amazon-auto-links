...
class AmazonAutoLinks_AdminPageFramework_FieldType_contact extends AmazonAutoLinks_AdminPageFramework_FieldType_submit {
    public $aFieldTypeSlugs = array( 'contact', );
    private $___sAction = 'amazon-auto-links_contact_field_type_email';
    ...
    {
        return array( array( 'handle_id' => 'amazon-auto-links-field-type-contact', 'src' => dirname(__FILE__) . '/js/contact.bundle.js', 'in_footer' => true, 'dependencies' => array( 'jquery', 'amazon-auto-links-script-form-main' ), 'translation_var' => 'AmazonAutoLinks_AdminPageFrameworkContactFieldType', 'translation' => array( 'nonce' => wp_create_nonce(get_class($this)), 'action' => $this->___sAction, 'messages' => array( 'requiredField' => $this->oMsg->get('please_fill_out_this_field'), ), ), ), );
    }
    ...
    private function ___doAjaxResponse()
    {
        ...
        $_oEmail = new AmazonAutoLinks_AdminPageFramework_FormEmail($_aEmailOptions, $_aInputs, $_sSubmitSectionID);
